...
"""
Editor : Sungsik Kong
Comments with THIS types are written by me, Sungsik Kong
I am really new to coding EKF algorithm questions, so I just wrote everything that I encountered as a hardship
Hope all the beginners watching my codes 'get helped'
"""
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + sensor_var*np.eye(3))

    # 3.2 Compute error state
    del_x = K @ (y_k - p_check)
    # del_x = K @ (y_k - p_cov_check)

    # 3.3 Correct predicted state
    p_hat = p_check + del_x[:3]
    v_hat = v_check + del_x[3:6]    
    
    del_phi = del_x[6:]
    # q_hat = Quaternion(axis_angle = angle_normalize(del_phi)).quat_mult_left(q_check)
    q_hat = Quaternion(*q_check).quat_mult_right(Quaternion(axis_angle=angle_normalize(del_phi)))
    q_hat = angle_normalize(q_hat)
    # q_hat = Quaternion(euler = del_phi).quat_mult_left(q_check)


    # 3.4 Compute corrected covariance
    p_cov_hat = (I - K @ h_jac) @ p_cov_check
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2)*(C_ns @ imu_f.data[k-1] + g)/2.
    v_est[k] = v_est[k-1] +delta_t*(C_ns @ imu_f.data[k-1] + g)
    q_est[k] = Quaternion(axis_angle = angle_normalize(imu_w.data[k-1] * delta_t)).quat_mult_right(q_est[k-1])
    q_est[k] = angle_normalize(q_est[k])
    
    # 1.1 Linearize the motion model and compute Jacobians

    # 2. Propagate uncertainty
    """
    if you check the given question illustration note of error state, you can find the shape of F, L, Q Jacobian by note!!
    plus, l_jac is predefined above !
    """
    F[:3, 3:6] = delta_t * np.eye(3)
    F[3:6, 6:] = (-1*delta_t)*skew_symmetric(C_ns @ imu_f.data[k-1])

    Q[:3, :3] = (delta_t**2)*var_imu_f*np.eye(3)
    ...
    # Update states (save)
    """
    sensor_var = var_gnss ? var_lidar?
    (if lidar or gnss available)
    + set the time interval of each sensors...
    first, please recall that we have each own updating schedule for lidar and gnss which is lidar_i and gnss_i
    then by checking whether the current time is matched with imu sensor time schedule, and then update it if matched

    here y_k can be the data driven from each sensor (where I got confused..!)
    """
    ...
    """
    FINAL CONFUSION on PART 1:
    you SHOULD update everything even before the 'measurement_update'!
    it is because you should get all the data even when the lidar and gnss sensor are not working.
    if you do not update those, the most recent value when measurement_update is firstly updated would remain zero as well, leading everything to converge to zero..
    """
